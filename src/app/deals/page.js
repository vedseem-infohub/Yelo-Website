...
export default function DealsPage() {
  ...
  const fetchProducts = useCallback(async (page = 1, reset = false) => {
    ...
    try {
      ...
      const response = await shopAPI.getProducts('deals', {
        page,
        limit: BATCH_SIZE,
        sort: selectedSort,
        ...
        <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-sm border-b border-gray-200">
          <div className="flex items-center gap-4 px-4 py-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Best offers & discounts</p>
            </div>
          </div>
        </div>
                ...
                <div className="px-1">
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-x-1 gap-y-4">
                    {displayItems.map((item, index) => {
                      ...
                      return (
                        <ProductCard
                          key={item.data._id || item.data.id || `deal-${index}`}
                          product={item.data}
                          showAddToBag={true}
                        />
                      )
