...
export default function NewArrivalsPage() {
  ...
  const fetchProducts = useCallback(async (page = 1, reset = false) => {
    ...
    try {
      ...
      const response = await shopAPI.getProducts('new-arrivals', {
        page,
        limit: BATCH_SIZE,
        sort: selectedSort,
  ...
  const filteredProducts = useMemo(() => {
    ...
    switch (selectedSort) {
      ...
      case 'newest':
        filtered.sort((a, b) => {
          const dateA = a.dateAdded ? new Date(a.dateAdded) : (a.createdAt ? new Date(a.createdAt) : new Date(0))
          const dateB = b.dateAdded ? new Date(b.dateAdded) : (b.createdAt ? new Date(b.createdAt) : new Date(0))
          return dateB - dateA
        })
        break
      case 'popular':
      ...
      <div className="min-h-screen bg-gray-50 pb-24 relative">
        ...
        <div className="sticky top-0 z-40 bg-white/95 backdrop-blur-sm border-b border-gray-200">
          <div className="flex items-center gap-4 px-4 py-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900">New Arrivals</h1>
              <p className="text-xs text-gray-500">Latest fashion pieces</p>
            </div>
          </div>
        </div>
